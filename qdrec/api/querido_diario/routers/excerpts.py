...
def create_excerpt_metadata(
    excerpt_metadata: schemas.ExcerptMetadataCreate, db: Session = Depends(get_db)
):
    db_excerpt_metadata = crud.get_excerpt_metadata_by_id(
        db, excerpt_id=excerpt_metadata.excerpt_id
    )
    if db_excerpt_metadata:
        raise HTTPException(
            status_code=400, detail="Excerpt Metadata already registered"
        )
...
def read_named_entities_by_id(
    excerpt_id: str, db: Session = Depends(get_db), skip: int = 0, limit: int = 100
):
...
def create_named_entity(
    named_entity: schemas.NamedEntityCreate, db: Session = Depends(get_db)
):
...
def read_vectors(
    excerpt_id: int, db: Session = Depends(get_db), skip: int = 0, limit: int = 100
):
