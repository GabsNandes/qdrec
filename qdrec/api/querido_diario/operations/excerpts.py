...
def read_excerpt(excerpt_id: str):
    session = DBSession()
    excerpts = session.query(DBExcerpts).get(
        excerpt_id
    )  # .filter(DBExcerptss.text_id == text_id).first()
    return excerpts
...
def read_excerpts(data: ExcerptReadData):
    ...
    excerpts = session.query(
        DBExcerpts.excerpt_id, DBExcerpts.excerpt_vector
    )  # DBExcerpts.excerpt_id, DBExcerpts.excerpt_vector
...
def get_entity(data: string, entity_type: string):
    ...
    for entity in entities:
        ...
        if (closest_match or substring) and entity["entity_type"] == entity_type:
            (
                print("For entry string:", data, "closest entity found:", closest_match)
                if closest_match
                else True
            )
