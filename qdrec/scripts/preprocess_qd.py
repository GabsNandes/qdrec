...
def spaced_letters(text):
    ...
    text = text.replace(
        "qui NTA-F Ei RA, 22 DE Ju LHO DE", "quinta-feira, 22 de Julho de"
    )
    text = text.replace("Ó R G Ã O RESPONSÁVE L SME", "ÓRGÃO RESPONSÁVEL SME")
    text = text.replace("I P VA", "IPVA")
    text = text.replace(
        "R E C U R S O S : P r o g r a m a E s c o l a r A u t ô n o m a d e G e s t ã o",
...
def dots_that_mess_segmentation(text):
    ...
    text = re.sub(
        "\. *(n|n[uú]mero)[ \.°º]+", " número ", text, flags=re.IGNORECASE
    )  # . N°
...
def find_occurrences(text: str, character: str) -> list:
...
def get_whole_words(subtext: str) -> str:
    ...
    return (subtext[first_pos:last_pos], first_pos, last_pos)
...
def find_dashes_and_replace_words(text: str, df_ptbr: pd.DataFrame) -> str:
    ...
    for dash in dashes_indexes:
        ...
        if not (contains_number(word=new_word)):
            if new_word_cleaned in words:
                text = "".join([text[:space_before], new_word, text[space_after:]])

    return text


def clean_text(text: str, window_size: int = 50) -> str:
        ...
        for i in range(0, dash_indexes_size):
            ...
            if i == 0:
                ...
                if start_dash_position < (dash_indexes[i - 1] + window_size):
                    start_dash_position = (
                        last_space_position  # dash_indexes[i-1]+window_size
                    )

            subtext = text[start_dash_position:end_dash_position]
            ...
            subtext = fix_spelling_in_answer(subtext)[
                0
            ]  # aqui entra a validação no google

            first_fragment = text[start_dash_position:first_space_position]
            ...
            if i == (dash_indexes_size - 1):
                ...
                last_fragment = text[last_space_position:next_dash_position]

            final_text += " ".join([first_fragment, subtext, last_fragment])
...
def read_dicionario_br() -> pd.DataFrame:
    ...
    header_list = ["Word"]
    df_ptbr = pd.read_csv(path, skiprows=0, names=header_list, header=None, sep=",")
    return df_ptbr


def clean_and_save(text_list: list, output_name: str = "excerpts_cleaned") -> None:
    date = datetime.datetime.now().strftime("%d%m%Y%H00")
    output_name += date
    with open(f"{output_name}.csv", "a+", encoding="utf-8") as f:
...
def pipeline(df_path: str) -> None:
    ...
    logging.basicConfig(filename=f"QD-scrap_google-{date}.log", level=logging.INFO)
    ...
    df["cleaned_text"] = df["cleaned_text"].apply(
        lambda txt: find_dashes_and_replace_words(txt, df_ptbr)
    )
...
def pipeline_multiprocess(df: pd.DataFrame) -> None:
    ...
    df_unique_texts["cleaned_text"] = df_unique_texts["cleaned_text"].apply(
        lambda txt: find_dashes_and_replace_words(txt, df_ptbr)
    )
