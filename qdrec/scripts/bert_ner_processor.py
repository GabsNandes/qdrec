...
def execute_csv(file):
    ...
    for index, row in df.iterrows():
        ...
        excerpt_metadata = ExcerptMetadataCreate(
            excerpt_id=row["excerpt_id"],
            uf=row["source_state_code"],
            cidade=row["source_territory_name"],
            tema=row["excerpt_subthemes"],
            data=row["source_created_at"],
        )
        db_gen = get_db()
        db = next(db_gen)
        ...
        if len(names) > 0:
            for name in names:
                item = NamedEntityCreate(
                    excerpt_id=name["excerpt_id"],
